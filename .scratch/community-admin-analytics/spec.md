...
> 2026-09-06 起部分不变量已由 `.scratch/community-round-2/spec.md`（D49–D52）取代：公开作品必须附带原图并存私有 COS；评论审核改由腾讯云文本内容安全承担，不再维护字面词版本；标签由管理员挂在作品上、打标不强制填理由；官方批次会随草稿上传原图。以下正文保留为当时的历史记录。
...
初次实现之后，基于 `6f44fbc` 复核登记的九项缺陷及追加并发/匿名化问题已在 `8c1d986`、`629937e` 修复。最终候选的覆盖率、Linux 专用测试、PostgreSQL 16 合约与迁移、三浏览器三轮 E2E、性能五轮、生产镜像与真实 HTTP 合约及双轴审查均通过；本地可实施事项已完成。详见 [本轮缺陷闭环与验收证据](../review-2026-09-05/spec.md)。上一轮记录保留为历史证据，不代替当前候选的验证结果。
