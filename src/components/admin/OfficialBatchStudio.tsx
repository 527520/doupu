...
type ItemStatus = 'pending' | 'running' | 'saved' | 'published' | 'failed' | 'cancelled' | 'unavailable';
...
export default function OfficialBatchStudio() {
  ...
  const [batch, updateBatch] = useState<BatchState | null>(null);
  const batchRef = useRef<BatchState | null>(null);
  const setBatch = useCallback((next: BatchState | null) => { batchRef.current = next; updateBatch(next); }, []);
  ...
  useEffect(() => {
    ...
    void fetch('/api/admin/batches').then(async (response) => {
      ...
      if (!mounted || !response.ok || itemsRef.current.length > 0) return;
      ...
      const restored: Item[] = latest.drafts.map((draft: { id: string; title: string; status: string }, index: number) => ({
        ...
        title: draft.title, cropInset: 0, paramsOverride: {}, status: draft.status === 'draft' ? 'saved' : draft.status === 'published' ? 'published' : 'unavailable', progress: 100, error: null, revisionId: draft.id, selected: false,
      }));
    ...
    }).catch(() => { if (mounted && itemsRef.current.length === 0) setMessage(t.actionFailed); });
  ...
  const selectFiles = (files: FileList | null) => {
    if (starting.current || dispatching.current) return;
  ...
  const processItem = async (item: Item, batchId: string) => {
    ...
    try {
      ...
      if (control.current.cancelled || itemsRef.current.find((entry) => entry.localId === item.localId)?.status === 'cancelled') throw new DOMException('Cancelled', 'AbortError');
  ...
  const dispatch = async (batchId: string) => {
    ...
    const workerCount = Math.min(itemsRef.current.filter((item) => item.status === 'pending').length, officialBatchConcurrency(navigator.hardwareConcurrency, deviceMemory));
    await Promise.all(Array.from({ length: workerCount }, async () => {
      while (!control.current.paused && !control.current.cancelled) {
        const item = itemsRef.current.find((candidate) => candidate.status === 'pending');
        if (!item) return;
        await processItem(item, batchId);
      }
    }));
    const current = itemsRef.current;
    if (!control.current.paused && !control.current.cancelled && current.every((item) => ['saved', 'published', 'failed', 'cancelled', 'unavailable'].includes(item.status))) {
      const failures = current.filter((item) => ['failed', 'cancelled'].includes(item.status)).length;
      const finished = await api(`/api/admin/batches/${batchId}`, { method: 'PATCH', headers: { 'content-type': 'application/json', 'idempotency-key': crypto.randomUUID() }, body: JSON.stringify({ action: 'finish', expectedVersion: batchRef.current?.version, reason }) });
      setBatch({ id: finished.id, version: finished.version, status: finished.status });
      track({ name: 'official_batch_completed', properties: { result: failures ? 'partial' : 'succeeded', itemCountBucket: countBucket(current.length) } });
  ...
  const start = async () => {
    if (starting.current || dispatching.current || batchRef.current || items.length === 0 || reason.trim().length < 3) return;
  ...
  const retry = async (item: Item) => {
    if (!batch || !item.file || dispatching.current) return;
    try {
      if (batch.status !== 'running') {
        const resumed = await api(`/api/admin/batches/${batch.id}`, { method: 'PATCH', headers: { 'content-type': 'application/json', 'idempotency-key': crypto.randomUUID() }, body: JSON.stringify({ action: 'resume', expectedVersion: batch.version, reason }) });
        setBatch({ id: resumed.id, version: resumed.version, status: resumed.status });
      }
      control.current = { paused: false, cancelled: false };
      patchItem(item.localId, { status: 'pending', error: null });
      await dispatch(batch.id);
    } catch (error) { setMessage(error instanceof Error ? error.message : t.transitionFailed); }
  };
