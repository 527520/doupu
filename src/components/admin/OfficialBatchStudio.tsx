...
export default function OfficialBatchStudio() {
  ...
  useEffect(() => {
    ...
    void fetch('/api/admin/batches').then(async (response) => {
      ...
      const restored: Item[] = latest.drafts.map((draft: { id: string; title: string; status: string }, index: number) => ({
        ...
        title: draft.title, cropInset: 0, paramsOverride: {}, status: 'saved', progress: 100, error: null, revisionId: draft.id, selected: false,
      }));
  ...
  const publish = async () => {
    ...
    try { const result = await api(`/api/admin/batches/${batch.id}/publish`, { method: 'POST', headers: { 'content-type': 'application/json', 'idempotency-key': crypto.randomUUID() }, body: JSON.stringify({ revisionIds, expectedVersion: batch.version, reason }) }); setBatch({ id: result.batch.id, version: result.batch.version, status: result.batch.status }); const failed = items.some((item) => item.status === 'failed'); track({ name: 'official_batch_completed', properties: { result: failed ? 'partial' : 'succeeded', itemCountBucket: countBucket(items.length) } }); setMessage(t.published(revisionIds.length)); } catch (error) { setMessage(error instanceof Error ? error.message : t.publishFailed); }
  };
