...
export function useAdminInspection<T>(url: string | null) {
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      if (sequence.current === request) setState({ url, data: null, error: controller.signal.aborted ? zhCN.communityAdmin.command.readTimeout : caught instanceof Error ? caught.message : zhCN.communityAdmin.queueLoadFailed });
