...
export default function ShoppingListPanel({ stats, designName, width, height }: Props) {
  ...
  return (
    <section aria-label={t.title} className="card-surface flex flex-col gap-2 p-3 text-sm">
      ...
      <p className="text-xs text-ink-soft">{t.summary(list.total, list.colors, list.packs, list.beadsPerPack)}</p>
      ...
      {open && (
        <>
          ...
          <p className="text-xs text-ink-soft">{t.packHint}</p>
          ...
          <ul className="flex max-h-56 flex-col gap-1 overflow-auto pr-1">
            {list.items.map((item) => (
              <li key={item.code} className="flex items-center gap-2 text-xs">
                ...
                <span className="ml-auto tabular-nums text-ink-soft">{t.packs(item.packs)}</span>
              </li>
            ))}
          </ul>
          ...
          <div className="flex items-center gap-2">
            <button type="button" onClick={() => void copy()} className="btn-outline btn-sm">
              {t.copy}
            </button>
            {copied === 'done' && <span className="text-xs text-success">{t.copied}</span>}
          </div>
          {copied === 'failed' && <Notice kind="warning" compact>{t.copyFailed}</Notice>}
        </>
      )}
    </section>
  );
