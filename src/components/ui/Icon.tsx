import type { ReactNode } from 'react';

export type IconName =
  | 'home' | 'spark' | 'folder' | 'palette' | 'user' | 'help' | 'info'
  | 'lock' | 'cloud' | 'upload' | 'blank' | 'arrow' | 'more' | 'plus'
  | 'search' | 'bell' | 'edit' | 'grid' | 'sliders' | 'download' | 'crop'
  | 'hand' | 'brush' | 'eraser' | 'zoom-in' | 'zoom-out' | 'fit'
  | 'undo' | 'redo' | 'mark' | 'back'
  | 'trash' | 'heart' | 'heart-filled' | 'flag' | 'check' | 'close' | 'refresh'
  | 'play' | 'pause' | 'stop' | 'eye' | 'tag' | 'external' | 'filter' | 'calendar'
  | 'image' | 'copy' | 'alert' | 'inbox' | 'users' | 'shield' | 'chart' | 'list'
  | 'minus' | 'log-out' | 'clock' | 'send' | 'star' | 'settings' | 'images'
  | 'chevron-down' | 'chevron-up' | 'chevron-left' | 'chevron-right';

const paths: Record<IconName, ReactNode> = {
  trash: <><path d="M4 7h16" /><path d="M9.5 7V4.5h5V7" /><path d="m6.5 7 .8 12.5h9.4L17.5 7" /><path d="M10 11v5.5M14 11v5.5" /></>,
  heart: <path d="M12 20.5s-7.5-4.6-7.5-10A4.2 4.2 0 0 1 12 8a4.2 4.2 0 0 1 7.5 2.5c0 5.4-7.5 10-7.5 10Z" />,
  'heart-filled': <path fill="currentColor" d="M12 20.5s-7.5-4.6-7.5-10A4.2 4.2 0 0 1 12 8a4.2 4.2 0 0 1 7.5 2.5c0 5.4-7.5 10-7.5 10Z" />,
  flag: <><path d="M5 21V4" /><path d="M5 4.5h11l-1.5 3.5L16 11.5H5" /></>,
  check: <path d="m5 12.5 4.5 4.5L19 7.5" />,
  close: <path d="m6 6 12 12M18 6 6 18" />,
  refresh: <><path d="M20 12a8 8 0 1 1-2.3-5.6" /><path d="M20 4v4.5h-4.5" /></>,
  play: <path d="M7 5v14l11-7L7 5Z" />,
  pause: <><rect x="6.5" y="5" width="3.5" height="14" rx="1" /><rect x="14" y="5" width="3.5" height="14" rx="1" /></>,
  stop: <rect x="6" y="6" width="12" height="12" rx="2" />,
  eye: <><path d="M2.5 12S6 5.5 12 5.5 21.5 12 21.5 12 18 18.5 12 18.5 2.5 12 2.5 12Z" /><circle cx="12" cy="12" r="3" /></>,
  tag: <><path d="M3.5 12.5v-8h8l9 9-8 8-9-9Z" /><circle cx="7.5" cy="8.5" r="1.2" /></>,
  external: <><path d="M14 4h6v6" /><path d="m20 4-9 9" /><path d="M19 14v6H4V5h6" /></>,
  filter: <path d="M4 5h16l-6.5 8v6l-3 1.5V13L4 5Z" />,
  calendar: <><rect x="3.5" y="5" width="17" height="15.5" rx="2" /><path d="M3.5 10h17M8 3v4M16 3v4" /></>,
  image: <><rect x="3.5" y="5" width="17" height="14" rx="2" /><circle cx="9" cy="10" r="1.6" /><path d="m20.5 16-5-5-8 8" /></>,
  images: <><rect x="6.5" y="7" width="15" height="12" rx="2" /><path d="M3 15V5.5A1.5 1.5 0 0 1 4.5 4H16" /><circle cx="11" cy="11" r="1.4" /><path d="m21.5 16.5-4-4-6 6" /></>,
  copy: <><rect x="9" y="9" width="11" height="11" rx="2" /><path d="M15 9V6a2 2 0 0 0-2-2H6a2 2 0 0 0-2 2v7a2 2 0 0 0 2 2h3" /></>,
  alert: <><path d="M12 3.5 22 20H2L12 3.5Z" /><path d="M12 10v4.5M12 17.5h.01" /></>,
  inbox: <><path d="M3.5 13.5 6 5.5h12l2.5 8v5.5h-17v-5.5Z" /><path d="M3.5 13.5H9a3 3 0 0 0 6 0h5.5" /></>,
  users: <><circle cx="9" cy="8" r="3.5" /><path d="M2.5 20a6.5 6.5 0 0 1 13 0" /><path d="M16 4.8a3.5 3.5 0 0 1 0 6.4" /><path d="M17.5 14a6 6 0 0 1 4 6" /></>,
  shield: <><path d="M12 3 4.5 6v6c0 4.5 3.2 7.7 7.5 9 4.3-1.3 7.5-4.5 7.5-9V6L12 3Z" /><path d="m9 12 2 2 4-4.5" /></>,
  chart: <><path d="M4 20h16" /><path d="M7 16v-5M12 16V7M17 16v-8" /></>,
  list: <><path d="M8 6h13M8 12h13M8 18h13" /><path d="M3.5 6h.01M3.5 12h.01M3.5 18h.01" /></>,
  minus: <path d="M5 12h14" />,
  'log-out': <><path d="M10 4H5v16h5" /><path d="m14 8 4 4-4 4M18 12H9" /></>,
  clock: <><circle cx="12" cy="12" r="9" /><path d="M12 7v5l3.5 2" /></>,
  send: <><path d="M21 3 3 10.5l7.5 2 2 7.5L21 3Z" /><path d="m10.5 12.5 4-4" /></>,
  star: <path d="m12 3.5 2.6 5.5 6 .7-4.4 4.1 1.1 5.9L12 16.8l-5.3 2.9 1.1-5.9-4.4-4.1 6-.7L12 3.5Z" />,
  settings: <><circle cx="12" cy="12" r="3" /><path d="M12 2.5v2M12 19.5v2M2.5 12h2M19.5 12h2M5.3 5.3l1.4 1.4M17.3 17.3l1.4 1.4M5.3 18.7l1.4-1.4M17.3 6.7l1.4-1.4" /></>,
  'chevron-down': <path d="m6 9 6 6 6-6" />,
  'chevron-up': <path d="m6 15 6-6 6 6" />,
  'chevron-left': <path d="m15 6-6 6 6 6" />,
  'chevron-right': <path d="m9 6 6 6-6 6" />,
  home: <><path d="M3.5 10.5 12 3l8.5 7.5" /><path d="M5.5 9.5v10h13v-10M9.5 19.5v-6h5v6" /></>,
  spark: <><path d="m12 2 1.7 5.2L19 9l-5.3 1.8L12 16l-1.7-5.2L5 9l5.3-1.8L12 2Z" /><path d="m19 15 .8 2.2L22 18l-2.2.8L19 21l-.8-2.2L16 18l2.2-.8L19 15Z" /></>,
  folder: <path d="M3 6.5h6l2-2h10v15H3v-13Z" />,
  palette: <><path d="M12 3a9 9 0 1 0 0 18h1.2a1.8 1.8 0 0 0 1.2-3.1 1.8 1.8 0 0 1 1.2-3.1H18A3 3 0 0 0 21 12a9 9 0 0 0-9-9Z" /><path d="M7.2 10h.1M9.5 6.8h.1M14 6.5h.1M17 9.2h.1" /></>,
  user: <><circle cx="12" cy="8" r="4" /><path d="M4.5 21a7.5 7.5 0 0 1 15 0" /></>,
  help: <><circle cx="12" cy="12" r="9" /><path d="M9.7 9a2.5 2.5 0 1 1 3.7 2.2c-.9.5-1.4 1-1.4 2.1M12 17h.01" /></>,
  info: <><circle cx="12" cy="12" r="9" /><path d="M12 11v6M12 7h.01" /></>,
  lock: <><rect x="4" y="10" width="16" height="11" rx="2" /><path d="M8 10V7a4 4 0 0 1 8 0v3" /></>,
  cloud: <path d="M7 18H6a4 4 0 1 1 1-7.9A6 6 0 0 1 18.5 12H19a3 3 0 0 1 0 6H7Z" />,
  upload: <><path d="M12 16V4m0 0L7.5 8.5M12 4l4.5 4.5" /><path d="M4 14v6h16v-6" /></>,
  blank: <><path d="M5 3.5h10l4 4v13H5v-17Z" /><path d="M15 3.5v4h4M8.5 14h7M12 10.5v7" /></>,
  arrow: <><path d="M5 12h14M14 7l5 5-5 5" /></>,
  more: <><circle cx="5" cy="12" r="1" /><circle cx="12" cy="12" r="1" /><circle cx="19" cy="12" r="1" /></>,
  plus: <path d="M12 5v14M5 12h14" />,
  search: <><circle cx="10.5" cy="10.5" r="6.5" /><path d="m15.5 15.5 5 5" /></>,
  bell: <><path d="M18 9a6 6 0 0 0-12 0c0 7-3 7-3 8h18c0-1-3-1-3-8Z" /><path d="M10 21h4" /></>,
  edit: <><path d="m4 20 4.5-1 10-10-3.5-3.5-10 10L4 20Z" /><path d="m13.5 7 3.5 3.5" /></>,
  grid: <><rect x="3" y="3" width="7" height="7" /><rect x="14" y="3" width="7" height="7" /><rect x="3" y="14" width="7" height="7" /><rect x="14" y="14" width="7" height="7" /></>,
  sliders: <><path d="M4 6h6M14 6h6M4 12h11M19 12h1M4 18h3M11 18h9" /><circle cx="12" cy="6" r="2" /><circle cx="17" cy="12" r="2" /><circle cx="9" cy="18" r="2" /></>,
  download: <><path d="M12 4v11m0 0-4-4m4 4 4-4" /><path d="M4 18v2h16v-2" /></>,
  crop: <><path d="M7 3v14a2 2 0 0 0 2 2h12" /><path d="M3 7h12a2 2 0 0 1 2 2v12" /></>,
  hand: <><path d="M7.5 11V6.5a1.5 1.5 0 0 1 3 0V10" /><path d="M10.5 10V4.5a1.5 1.5 0 0 1 3 0V10" /><path d="M13.5 10V6a1.5 1.5 0 0 1 3 0v5" /><path d="M16.5 11V8.5a1.5 1.5 0 0 1 3 0V14c0 4.4-2.6 7-7 7h-1c-2.2 0-3.7-.9-5-2.6L3.8 15a1.6 1.6 0 0 1 2.4-2.1L8 14.5" /></>,
  brush: <><path d="m14.5 4.5 5 5L10 19H5v-5l9.5-9.5Z" /><path d="m12.5 6.5 5 5" /><path d="M5 19c-1.5 0-2.5.7-3 2 1.7.2 3.4-.1 4.5-1.5" /></>,
  eraser: <><path d="m8.5 5.5 10 10-4.5 4.5H8l-5-5 10-10a3.2 3.2 0 0 1 4.5 0" /><path d="m7 11 6 6" /></>,
  'zoom-in': <><circle cx="10.5" cy="10.5" r="6.5" /><path d="m15.5 15.5 5 5M10.5 7.5v6M7.5 10.5h6" /></>,
  'zoom-out': <><circle cx="10.5" cy="10.5" r="6.5" /><path d="m15.5 15.5 5 5M7.5 10.5h6" /></>,
  fit: <><path d="M8 3H3v5M16 3h5v5M8 21H3v-5M16 21h5v-5" /><path d="M8 8h8v8H8z" /></>,
  undo: <><path d="M9 7 4 12l5 5" /><path d="M5 12h8a6 6 0 0 1 6 6" /></>,
  redo: <><path d="m15 7 5 5-5 5" /><path d="M19 12h-8a6 6 0 0 0-6 6" /></>,
  mark: <><circle cx="12" cy="12" r="8" /><path d="m8.5 12 2.2 2.2 4.8-5" /></>,
  back: <><path d="m14 5-7 7 7 7" /><path d="M7 12h13" /></>,
};

export default function Icon({ name, size = 20, className }: { name: IconName; size?: number; className?: string }) {
  return <svg aria-hidden="true" className={className} viewBox="0 0 24 24" width={size} height={size} fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round">{paths[name]}</svg>;
}
