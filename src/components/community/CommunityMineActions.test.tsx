...
it('撤回本次审核不会调用撤回整件作品，操作先确认', async () => {
  view(); fireEvent.click(screen.getByRole('button', { name: '撤回审核' }));
...
it('整件作品撤回明确说明公开版与待审版都会隐藏，失败保留弹窗可重试', async () => {
  ...
  view(); fireEvent.click(screen.getByRole('button', { name: '撤回作品' }));
...
it('草稿可直接提交已确认许可的冻结内容，重复点击只发送一次', async () => {
  view('draft'); const button = screen.getByRole('button', { name: '提交审核' });
...
it('已发布版本提供修改重提入口，并携带私有选源上下文', () => {
  view('published'); expect(screen.getByRole('link', { name: '修改后重投' })).toHaveAttribute('href', '/community/submit?workId=work&designId=source');
});

it('不把不完整成功响应当成已完成，保留原动作并阻止切换撤回范围', async () => {
  state.fetch.mockResolvedValueOnce(new Response('{}'));
  view(); fireEvent.click(screen.getByRole('button', { name: '撤回审核' })); fireEvent.click(screen.getByRole('button', { name: '确认撤回' }));
...
it('未知提交的目标和基准版本不随新 props 改变，禁止切换为撤回', async () => {
  ...
  const rendered = view('draft'); fireEvent.click(screen.getByRole('button', { name: '提交审核' }));
  ...
  expect(screen.getByRole('button', { name: '撤回并修改' })).toBeDisabled();
  fireEvent.click(screen.getByRole('button', { name: '重试确认' }));
  await waitFor(() => expect(state.refresh).toHaveBeenCalled());
...
it('确定的版本冲突允许关闭，不复用失败动作的键', async () => {
  ...
  fireEvent.click(screen.getByRole('button', { name: '撤回作品' })); fireEvent.click(screen.getByRole('button', { name: '确认撤回' }));
  await waitFor(() => expect(state.refresh).toHaveBeenCalled());
...
it('确认成功后刷新失败也不允许重放写操作', async () => {
  ...
  view('draft'); fireEvent.click(screen.getByRole('button', { name: '提交审核' }));
  await screen.findByRole('alert');
