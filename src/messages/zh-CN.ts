...
export const zhCN = {
  ...
  communityAdmin: {
    ...
    command: {
      ...
      saved: '操作已完成。', loading: '正在读取…', reload: '重新读取', back: '返回队列', select: '选择对象后再处置',
      empty: '没有符合条件的对象。', cancel: '取消编辑', save: '保存修改',
      previous: '当前已公开版本（对照）', frozenMaterial: '本次冻结图纸', stale: '对象版本已变化，请刷新后重新确认。',
    },
    ...
    community: {
      ...
      rules: '阅读社区规则', searchLabel: '搜索作品标题', searchPlaceholder: '搜索作品标题', sortLabel: '排序',
    ...
    pages: {
      ...
      works: { eyebrow: 'WORKS / GOVERNANCE', title: '作品管理', description: '先核对冻结作品与当前状态，再精选、管理评论或下架；恢复只使用已批准版本。' },
    ...
    batch: {
      ...
      status: { pending: '待处理', running: '生成中', saving: '保存中', save_unknown: '保存结果待确认', saved: '已保存', published: '已发布', failed: '失败', cancelled: '已取消', unavailable: '已撤回或不可发布' },
    },
    reviewFailed: '审核失败',
    works: { queue: '作品列表', material: '作品材料与处置', search: '搜索标题、作者或完整作品 ID', status: '作品状态', all: '全部状态', query: '查询作品', noTitle: '暂无修订标题', featured: '已精选', page: (page: number) => `第 ${page} 页`, previous: '上一页', next: '下一页',
      public: '公开可见', notPublic: '未公开', locked: '评论已锁定', unlocked: '评论可互动', removedReason: '下架理由：', noMaterial: '作品暂时没有可读取的修订材料。', newerRevision: '另有较新修订：', counts: (likes: number, comments: number, reuses: number) => `${likes} 次点赞 · ${comments} 条公开评论 · ${reuses} 次引用`, openPublic: '在新窗口查看公开页', reason: '操作理由',
    ...
    rules: { harm: '明确伤害', harassment: '骚扰', sexual: '色情', spam: '垃圾推广', current: '当前规则', history: '历史', newVersion: '新规则版本', literalOnly: '只接受字面词；匹配后进入人工审核，不自动封号。', literal: '字面词', category: '伤害分类', add: '加入版本', remove: '移除', reason: '启用理由', publish: '创建并启用不可变版本', historyTitle: '版本历史', ruleCount: '条规则',
      bundled: '仓库初始词表', editCurrent: '基于当前版本编辑', select: '先核对当前词表，再创建新版本。', historyLimit: '展示最近 50 个不可变版本及完整词条。', replaceHelp: '新版本会替换整个现行词表，不是追加单个词。旧版本保留为历史记录，结构规则继续生效。',
      confirmReplace: '我已核对全部词条，确认使用此完整词表替换当前版本。', duplicate: '该分类已包含相同字面词。', noEmpty: '不能启用空词表，请保留至少一条规则。', restart: '放弃当前编辑并载入最新词表', baseVersion: (version: number) => version === 0 ? '基于仓库初始词表编辑' : `基于 v${version} 编辑`,
    },
    tags: { title: '正式标签', name: '名称', slug: '链接标识', reason: '操作理由', create: '创建标签', createTitle: '创建正式标签', sort: '排序', action: '标签处置', enabled: '启用', disabled: '停用', mergedState: '已合并', mergedTo: '已合并到',
      slugHelp: '使用小写字母、数字和连接符，例如 animals；越小的排序值越靠前。', merge: '合并重复标签', mergeTarget: '合并到标签', chooseTarget: '选择现有正式标签', mergeSubmit: '确认合并标签',
      mergeHelp: '合并会迁移全部修订关联，旧标签停用并保留指向。此操作不能通过改名撤销。', confirmMerge: (source: string, target: string) => `我确认将「${source}」合并到「${target}」`,
    },
    users: { anonymized: '已注销用户', unverified: ' · 未验证', restore: '恢复账号', suspend: '暂停账号', title: '账号与角色', search: '搜索账号', query: '查询', reason: '操作理由', confirmation: '目标 userId 二次确认', accountId: '内部账号 ID', email: '脱敏邮箱', role: '当前角色', nextRole: '调整为角色', status: '账号状态', action: '人员处置', changeRole: '确认调整角色',
      searchHelp: '按用户名、邮箱或完整 userId 查询；每次最多显示最近 100 个匹配账号。', selfProtected: '这是当前登录账号，不能修改自己的角色或暂停自己。', anonymizedProtected: '账号已不可恢复匿名化，不能修改角色或重新启用。',
      impact: '角色或账号状态变更会撤销目标账号的全部会话。暂停只阻断登录与新增操作，不会自动下架既有作品。请核对账号 ID 并填写理由。',
      adminWarning: '管理员可以查看分析、管理人员、审核规则与官方内容。请再次确认这是需要授予完整管理权限的账号。',
    },
