...
const schema = z.object({ action: z.enum(['pause', 'resume', 'cancel']), expectedVersion: z.number().int().positive(), reason: z.string() }).strict();
async function patch(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await executeIdempotently(getDb(), { actorUserId: actor.userId, scope: `admin.official-batch.transition:${batchId}`, key: request.headers.get('idempotency-key') ?? '', request: input },
