...
async function post(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await executeIdempotently(getDb(), { actorUserId: actor.userId, capability: 'official:manage', scope: `admin.official-batch.publish:${batchId}`, key: request.headers.get('idempotency-key') ?? '', request: input },
