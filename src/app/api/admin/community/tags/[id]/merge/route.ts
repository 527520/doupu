...
async function post(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await executeIdempotently(getDb(), {
    actorUserId: actor.userId, capability: 'community:moderate', scope: `admin.community.tag.merge:${sourceTagId}`,
    key: request.headers.get('idempotency-key') ?? '', request: input,
  }, (tx) => mergeCommunityTag(tx, { actor, sourceTagId, requestId, ...input }));
