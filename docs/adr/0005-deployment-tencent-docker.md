...
- Nightly verified `pg_dump` via the `backup` sidecar's fail-fast scheduling loop, uploaded to 腾讯云 COS under the `doupu-backup/` prefix; restore procedure documented in `deploy/`. The original 30-day COS lifecycle rule was retired by ADR-0019 (the bucket is now shared with work originals, which must never auto-expire); old backups are pruned manually. Backup or alert-path failure exits non-zero after bounded container retries instead of sleeping silently.
